...
public class GraphRepositoryImpl implements IGraphRepository {
	...
	@Override
	public int insertGraph(User u, String graph_data, String title, String description, boolean is_public) {
	...
	public void updateGraph(Graph g) {
		String update_graph = "UPDATE Graph"
				...
				+ "                   last_modified = NOW()"
				+ "             WHERE id = ?";
		access.query(update_graph, g.getGraph_data(), g.getTitle(), g.getDescription(), g.is_template(), g.getId());
	}

	@Override
	public List<Graph> getUserGraphs(User u) {
		List<Graph> graphs = new ArrayList<Graph>();
		String query = "SELECT graph_data, Graph.id, title, description, is_public, is_template, created, last_modified"
				+ "       FROM Graph"
				+ "            LEFT JOIN Role"
				+ "            ON Graph.id = Role.id"
				+ "      WHERE Role.user_id = ?";
		try {
			...
			while (rs.next()) {
				...
				graphs.add(new Graph(id, graph_data, title, description, is_public, is_template, created, last_modified));
			}
		} catch (SQLException e ) {
	...
	public Graph getUserGraph(User u, int id) {
		Graph graph = null;
		String query = "SELECT graph_data, title, description, is_public, is_template, created, last_modified"
				+ "       FROM Graph"
				+ "            LEFT JOIN Role"
				+ "            ON Graph.id = Role.id"
				+ "      WHERE Role.user_id = ?"
		...
		try {
			...
			while (rs.next()) {
				...
				Date created = rs.getDate("created");
				Date last_modified = rs.getDate("last_modified");
				graph = new Graph(id, graph_data, title, description, is_public, is_template, created, last_modified);
			}
		} catch (SQLException e) {
			System.out.println("SQL error: " + e.getMessage());
		}
	...
	public List<Graph> getTemplateGraphs() {
		List<Graph> graphs = new ArrayList<Graph>();
		String query = "SELECT graph_data, id, title, description, is_public, is_template, created, last_modified"
				+ "       FROM Graph"
