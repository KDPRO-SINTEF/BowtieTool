...
package no.ntnu.kundestyrt.bowtie.servlets;
 ...
 *
 * <p>open.html implements the user interface. This file is displayed within an IFRAME in order to
 * better handle the response. The form is then processed either locally if the browser implements
 * the HTML5 FileReader API or via the OpenServlet. Note that the mechanism to open files uses
 * OpenFile in Editor.js, as well as Editor.openFile when the client starts. This is required to
 * abstract away the asynchronous loading of the new editor and handling of the response, which in
 * turn calls the setData method on the OpenFile instance of the parent window of the frame where
 * open.html was displayed (see below).
 */
public class OpenServlet extends HttpServlet {
  ...
  /**
   * Parses the given multipart/form-data request into a map that maps from names to values. Note
   * that this implementation ignores the file type and filename and does only return the actual
   * data as the value for the name of the file input in the form. Returns an empty map if the form
   * does not contain any multipart/form-data.
   */
  protected Map<String, String> parseMultipartRequest(HttpServletRequest request)
      throws IOException {
    ...
    if (contentType != null && contentType.indexOf("multipart/form-data") == 0) {
      ...
      Iterator<String> it =
          splitFormData(readStream(request.getInputStream()), boundary).iterator();
  ...
  /**
   * Parses the values in the given form-data part into the given map. The value of the name
   * attribute will be used as the name for the data. The filename will be stored under filename in
   * the given map and the content-type is ignored in this implementation.
   */
